...
```{r survey123_xlsform, echo=FALSE, fig.cap="Screen capture of ESRI Survey123 Connect software demonstrating the use of the XLSForm spreadsheet-based coding language and the ability to directly edit the JavaScript code that is linked to XLSForm parameters.", out.width = '90%'}
knitr::include_graphics("figures/survey123/survey123_xlsform.png", error = FALSE)
```
```{r survey123_xlsform2, echo=FALSE, fig.cap="Screen capture of ArcGIS website description of the XLSForm features: https://gis.idaho.gov/wp-content/uploads/2021/03/ArcGIS-Apps-for-the-Field-State-of-ID.pdf.", out.width = '90%'}
knitr::include_graphics("figures/survey123/survey123_xlsform2.png", error = FALSE)
```

```{r display hrs per this pp, echo = FALSE, include=TRUE,  message=FALSE, warning=FALSE  }

library(kableExtra)

df_pp %>% filter(Yr == mxyr ) %>% filter(wknum ==max(wknum)) %>%
  #arrange(Budget_Cat)%>%
  kbl() %>%
  #kable_styling()
